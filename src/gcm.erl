...
push(Name, RegIds, Message) ->
    ?LOG(info, "Pushing for name ~p registration id ~p message ~p", [Name, RegIds, Message]),
    push(Name, RegIds, Message, ?RETRY).

push(Name, RegIds, Message, Retry) ->
    ?LOG(info, "Pushing for name ~p registration id ~p message ~p for retires ~p", [Name, RegIds, Message, Retry]),
...
init([Key]) ->
    ?LOG(info, "Initializing gcm.erl ~p", [Key]),
    {ok, #state{key=Key}}.
...
handle_cast({send, RegIds, Message, Retry}, #state{key=Key} = State) ->
    ?LOG(info, "Pushing for retry ~p registration id ~p message ~p Handle cast", [RegIds, Message, Retry]),
    do_push(RegIds, Message, Key, Retry),
    {noreply, State};

handle_cast(_Msg, State) ->
    ?LOG(info, "No push for message ~p state ~p Handle cast", [_Msg, State]),
    {noreply, State}.
...
do_push(RegIds, Message, Key, Retry) ->
    ?LOG(info, "Sending message: ~p to reg ids: ~p retries: ~p.~n", [Message, RegIds, Retry]),
...
do_backoff(RetryAfter, RegIds, Message, Retry) ->
    case RetryAfter of
        no_retry ->
            ok;
        _ ->
            ?LOG(info, "Received retry-after. Will retry: ~p times~n", [Retry-1]),
