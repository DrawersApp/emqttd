...
dispatch(Msg = #mqtt_message{topic = Topic}, Session = #session{client_pid = undefined, message_queue = Q, client_id = ClientId}) ->
    emqttd_pubsub:publish(#mqtt_message{from = offline, topic = atom_to_binary(drawersoffline, utf8), qos = 1, payload = list_to_binary([ClientId, list_to_binary("%%"), Topic]), retain = false}),
